...
- **Мандаторні тести кожної CRUD-задачі:** **get/update/delete** неіснуючого id →
  `NOT_FOUND` (усі три, не лише update/delete — get теж); форма DTO
  (`Object.keys().sort()` — без витоку полів БД); форма елемента `listX()`; де є
  clearable-поле — персистенція явного `null`; де вхід — сума в грн — некоректна
  сума → `VALIDATION_FAILED` (не 500). Кожна експортована функція має бути
  викликана хоча б одним тестом.
